...
/*	model : DB 속의 데이터가 담길 곳 
	...
	protected void buildExcelDocument(Map<String, Object> model, Workbook workbook, HttpServletRequest request,
		...
		createdColumnLabel(sheet, columnLabels);
	...
	// 제목 
	private void createdColumnLabel(Sheet sheet, List<String> titles) {
		Row row = sheet.createRow(0); 
		Cell cell; 
		for(int i=0; i<titles.size(); i++) {
			sheet.setColumnWidth(i, 265*15);  // 컬럼폭 
	...
	private void createColumnValue(Sheet sheet, List<Object[]> values) {
		...
		for(int i=0; i<values.size(); i++) {
			...
			for(int col=0; col<oo.length; col++) {
				...
				if(oo[col] instanceof Short) {			// 해당하는 자료형으로 다운 캐스팅 
