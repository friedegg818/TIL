...
<tr height="37" style="background: #ffffff;">
    ...
    <td align="left" colspan="3" style="padding-left: 5px;"><span>${dto.tel}</span></td>
</tr>
<tr height="37" style="background: #ffffff;">
...
<tr height="37" style="background: #ffffff;">
    ...
    <td align="left" colspan="3" style="padding-left: 5px;">
	<select name="membership" class="selectField">
		<c:forEach var="vo" items="${listRole}">
			<c:if test="${vo.membership<sessionScope.member.membership}">
				<c:if test="${vo.membership!=0}">
					<option value="${vo.membership}" ${dto.membership==vo.membership?"selected='selected'":""}>${vo.memberRole}</option>
				</c:if>
			</c:if>
		</c:forEach>
	</select>
