...
function searchList() {
	...
	f.membership.value=$("#selectMembership").val();
	f.enabled.value=$("#selectEnabled").val();
	f.action="<%=cp%>/admin/memberManage/listMember";
	f.submit();
}
     ...
     <div>
			<table style="width: 100%; margin: 20px auto 0px; border-spacing: 0px;">
			   <tr height="35">
			      ...
			      <td align="right">
			          <select id="selectMembership" class="selectField" onchange="searchList();">
			              <option value="">:: 역할 ::</option>
			              <c:forEach var="vo" items="${listRole}">
			                  <c:if test="${vo.membership!=0 && vo.membership<99}">
			                      <option value="${vo.membership}" ${membership!=''&&membership==vo.membership?"selected='selected'":""}>${vo.memberRole}</option>
			                  </c:if>
			              </c:forEach>
			          </select>
			...
			<table style="width: 100%; margin: 10px auto; border-spacing: 0px;">
			   <tr height="40">
			      <td align="left" width="100">
			          <button type="button" class="btn" onclick="javascript:location.href='<%=cp%>/admin/memberManage/listMember';">새로고침</button>
			      </td>
			      <td align="center">
			          <form name="searchForm" action="<%=cp%>/admin/memberManage/listMember" method="post">
