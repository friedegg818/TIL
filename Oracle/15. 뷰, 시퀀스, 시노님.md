...
### VIEW 의 장점 
 - 자주 사용되는 복잡한 질의를 뷰로 정의하여 간단하게 표현 할 수 있다. 
 - 데이터 무결성을 보장하는데 활용 된다.
   - **WITH CHECK OPTION** : 뷰를 통해 수행되는 INSERT문과 UPDATE문이 뷰가 선택 할 수 없는 튜플들을 생성 할 수 없도록 보장
 ...
 - 데이터 독립성을 제공하고, 원본에 직접 접근하는 대신 뷰를 통해 데이터에 접근하도록 하기 때문에 보안 기능을 제공한다.    
   → 뷰가 많이 활용되는 이유 
 - 동일한 데이터에 대한 여러가지 뷰를 제공한다. 
