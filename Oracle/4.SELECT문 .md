...
### SELECT
 - 테이블이나 뷰에 있는 데이터를 조회하여 0개 이상의 행을 반환 
 - 가장 일반적으로 사용되는 DQL 명령 
 - DML의 구성 요소로 간주 되기도 함 (in 정보처리) 
...
### 기본 형식
	...
	· 특정 컬럼이나 표현식 출력 
        ...
        → 존재하지 않는 컬럼명을 입력 할 경우 'ORA-00904' 오류 발생		
		  * ORA-00904 : 부적합한 식별자 
	    → 수식 사용 가능  	  
	...
	· 모든 컬럼 출력 
	    SELECT * FROM emp; 
		→ 모든 컬럼을 출력하는 경우에도 * 대신 모든 컬럼명을 기술 하는 것이 좋음 
		  더 정확하고 에러를 없앨 수 있다. 
 ...
 ** 기초 함수 
 ...
 - DECODE 함수 
   ...
   · CASE 보다는 성능이 좋지 않음 , but 간단 
	...
	SELECT name, rrn, DECODE(SUBSTR(rrn,8,1),1,'남자',2,'여자') 
	FROM emp;
