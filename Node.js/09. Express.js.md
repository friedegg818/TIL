...
### Express 외부 모듈 설치
   ...
   1. npm을 초기화
	      ...
	      잘 모르는 경우 전부 그대로 Enter 키 >  package.json 파일 생성된다.
   ...
   2. Express 설치
	       ...
	       이 후 app 디렉토리에서 npm install 을 실행하면 종속 항목 목록 내의 모듈이 자동으로 설치
             --save 는 생략 가능

            응용 프로그램의 "nodejs"에 "node_modules"라는 폴더가 만들지고, 그 안에 'express'라는 폴더가 저장된다. 이것이 Express 파일이 저장되는 폴더이다.
