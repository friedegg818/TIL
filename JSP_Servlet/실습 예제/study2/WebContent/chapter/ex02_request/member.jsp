...
function sendOk() {
	...
	// 아이디
	if (!/^[a-z][a-z0-9_]{4,9}$/i.test(f.userId.value)) {			// . 앞에 i : 대소문자 구분 x
		alert("아이디는 5~10자 이내이며 영문자,숫자만 가능합니다.(첫글자는 영문자여야 함)");
		f.userId.focus();
		return;		
	}	
	
	// 비밀번호
	/* if(! /^(?=.*?[a-z])(?=.*?[0-9])(?=.*?[#?!@$%^&*-]).{5,10}$/i.test(f.userPwd.value)) { 숫자와 특수문자 모두 포함	 */
	if(! /^(?=.*[a-z])(?=.*[#?!@$%^&*\-+=]|.*[0-9]).{5,10}$/i.test(f.userPwd.value)) {
		alert("비밀번호는 5~10자 이내이며 영문자, 숫자 또는 특수문자가 포함 되어야 합니다.");
		f.userPwd.focus();
		return;
	}
	...
	if(! isValidKorean(f.userName.value)) {	
		f.userName.focus();
		return;
	}
...
function changeEmail() {	// 선택한 이메일 주소가 자동 입력되도록 
	var f = document.memberForm;
	
	var s = f.selectEmail.value;
	if(s != "direct") {	// 직접입력이 아니면
		f.email2.value = s;
...
<div class="member">
	...
	<div class="member-body">
		<form name="memberForm" action="member_ok.jsp" method="post">
			<table>
			<tr class="list-row-2">
			...
			<tr class="list-row-1">
				...
				<td style="padding-left: 5px;">
					...
					<button type="button" class="btn" onclick="daumPostcode();">우편번호검색</button></a>
				</td>
			</tr>
